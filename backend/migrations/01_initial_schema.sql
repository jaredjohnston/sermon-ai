...
CREATE TABLE public.content_templates (
  ...
  content_type_name text NOT NULL CHECK (char_length(content_type_name) >= 2 AND char_length(content_type_name) <= 50),
  structured_prompt text NOT NULL CHECK (char_length(structured_prompt) >= 50 AND char_length(structured_prompt) <= 5000),
  deleted_at timestamp with time zone,
  deleted_by uuid,
  created_by uuid NOT NULL,
  updated_by uuid NOT NULL,
  ...
  status USER-DEFINED NOT NULL DEFAULT 'active'::template_status,
  model_settings jsonb DEFAULT '{"model": "gpt-4o", "max_tokens": 2000, "temperature": 0.7}'::jsonb,
  created_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
  updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()),
  CONSTRAINT content_templates_pkey PRIMARY KEY (id),
  CONSTRAINT content_templates_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id),
  CONSTRAINT content_templates_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id),
  CONSTRAINT content_templates_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES auth.users(id),
...
CREATE TABLE public.generated_content (
  ...
  CONSTRAINT generated_content_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id),
  CONSTRAINT generated_content_transcript_id_fkey FOREIGN KEY (transcript_id) REFERENCES public.transcripts(id),
  CONSTRAINT generated_content_template_id_fkey FOREIGN KEY (template_id) REFERENCES public.content_templates(id),
  CONSTRAINT generated_content_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES auth.users(id),
  CONSTRAINT generated_content_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id),
  CONSTRAINT generated_content_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id)
);
CREATE TABLE public.media (
  client_id uuid NOT NULL,
  filename text NOT NULL CHECK (char_length(filename) >= 1 AND char_length(filename) <= 255),
  storage_path text NOT NULL UNIQUE CHECK (char_length(storage_path) >= 1 AND char_length(storage_path) <= 512),
  content_type text NOT NULL CHECK (content_type ~ '^(video|audio|text|application)/[\w\.-]+$'::text),
  size_bytes bigint NOT NULL CHECK (size_bytes > 0),
  deleted_at timestamp with time zone,
  deleted_by uuid,
  created_by uuid NOT NULL,
...
CREATE TABLE public.transcripts (
  ...
  user_id uuid NOT NULL,
  CONSTRAINT transcripts_pkey PRIMARY KEY (id),
  CONSTRAINT transcripts_media_id_fkey FOREIGN KEY (video_id) REFERENCES public.media(id),
  CONSTRAINT transcripts_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id),
  CONSTRAINT transcripts_deleted_by_fkey FOREIGN KEY (deleted_by) REFERENCES auth.users(id),
  CONSTRAINT transcripts_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id),
  CONSTRAINT transcripts_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES auth.users(id),
